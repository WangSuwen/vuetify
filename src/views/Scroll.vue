    ...
    <v-container>
      <v-layout row wrap>
        ...
        <v-flex xs12>
          ...
          <v-slider v-model="duration" min="0" max="1000" label="滚动时长" thumb-label></v-slider>
          <v-slider v-model="offset" min="-500" max="500" label="滚动偏移量" thumb-label></v-slider>
        </v-flex>
        <v-flex>
          <v-btn id="first" ref="button" color="primary" block @click="$vuetify.goTo(target, options)">scroll</v-btn>
        </v-flex>
      </v-layout>
    </v-container>
  </v-app>
